...
const Rooms = ({ navigation }) => {
  ...
  const handleRequestRoom = async (values) => {
    try {
      ...
      const response = await axios.post(`${baseUrl}/bookings`, {
        ...
        //   amount_of_people: values.amount_of_people,
  ...
  const requestRoomSchema = Yup.object({
    ...
    roommates: Yup.array()
      .of(Yup.string().email("Enter a valid email!"))
    ...
    <View style={{ flex: 1, backgroundColor: white, minHeight: "100%" }}>
      <ScrollView style={{ flex: 1 }}>
        <View style={styles.container}>
          <View style={styles.contentContainer}>
            <Text style={styles.reqRoomTitle}>
                      ...
                      {roommateFields.map((_, index) => (
                        <View key={index}>
                          <TextInput
                            ...
                            onChangeText={(text) => {
                              const newRoommates = Array.isArray(
                                values.roommates
                              )
                                ? [...values.roommates]
                                : new Array(roommateFields.length).fill("");
                            ...
                            autoCapitalize="none"
                          />
                          {errors.roommates?.[index] &&
                          touched.roommates?.[index] ? (
                            <Text style={styles.errorText}>
                              {typeof errors.roommates === "string"
                                ? errors.roommates
                                : errors.roommates[index]}
                            </Text>
                          ) : null}
      ...
      <Portal>
        ...
        >
          ...
          <Text style={styles.modalText}>Ваши данные успешно отправлены.</Text>
